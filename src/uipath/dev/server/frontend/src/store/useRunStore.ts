...
interface RunStore {
  ...
  stateEvents: Record<string, { node_name: string; timestamp: number }[]>;
  addStateEvent: (runId: string, nodeName: string) => void;
  ...
  addStateEvent: (runId, nodeName) =>
    set((state) => {
      ...
      return {
        stateEvents: {
          ...state.stateEvents,
          [runId]: [...existing, { node_name: nodeName, timestamp: Date.now() }],
        },
      };
    }),
