...
- A framework config file with agents/functions defined (e.g. `uipath.json`, `pydantic_ai.json`, `langgraph.json`)
- `entry-points.json` - Agent definitions (auto-generated by `uv run uipath init`)
...
**Parameters:**
- `<name>` - Agent/function name — the key from the active framework config (e.g., `"agent"` from `pydantic_ai.json`, `"main"` from `uipath.json`). This is the short name, **not** the file path. Check which config file has `"agents"` or `"functions"` defined.
